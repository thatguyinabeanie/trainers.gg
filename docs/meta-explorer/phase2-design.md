...
trustworthy dot) so a sleeper with 4 players doesn't read as loudly as one with 200.
...
over events that have _any_ non-NULL placement (placement-bearing events), and
...
communicates this with a small caption on the card header: _"Always compares all
sources — ignores the Source filter."_ When the user has narrowed the Source
...
- Denominator is computed **per source**: `SUM(total_players)` over
  `DISTINCT (event_key, division)` _within each source_. So `usage_pct` is
...
_measured_ slow** — matches the Phase 1 "indexes-only, escalate on measurement"
...
- Chart 3 (source dumbbells): **ignores `source`** by design — communicated via
  the card caption described above. (Chart 4, overall-vs-Top-10%, _does_ honor
  `source` — it compares overall vs the Top-10% field _within_ the selected
  source.)
