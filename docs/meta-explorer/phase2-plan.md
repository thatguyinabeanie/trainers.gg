...
source**: `SUM(total_players)` over `DISTINCT (event_key, division)` _within each
source_.
...
_measured_ slow — out of scope for this task.
...
export async function fetchUsageBySource(
  ...
  /* resolve minPlayers=0; call getCachedUsageBySource */
}
...
export async function fetchUsageConversion(
  ...
  /* resolve source="all", minPlayers=0, topPct=0.10; call getCachedUsageConversion */
}
```
...
export function coerceTopPct(raw: string | null | undefined): TopPct {
  /* parse float, fallback DEFAULT_TOP_PCT */
}
```
...
- **Ignores the Source filter (by design).** Render the caption via
  `data-chart-card`'s caption slot: _"Always compares all sources — ignores the
  Source filter."_ When the sidebar source filter is narrowed, add a subtle muted
...
- Click a line → `onSpeciesClick` (toggle in shared selection) — bump is a Trends
  ...
  Decision 5: sidebar selection drives line/stream/bump. The bump _honors_ the
...
- New indexes / materialized views — only if a query is _measured_ slow (it isn't
  yet).
