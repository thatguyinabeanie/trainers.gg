...
| Macro-layout            | ⚠️ **Superseded for the calc-OFF view** by [`single-column-revision.md`](./single-column-revision.md) — now a **single centered vertical column** (hero → stats → moves). Original: **Immersive, card-less, sprite-center flanked** (validated visually). No wrapping card. Three zones: stats panel (left) · sprite+identity+loadout (center) · moves panel (right). Carousel along the **bottom** of the canvas.                                                                |
...
> ⚠️ **Superseded for the calc-OFF view** by [`single-column-revision.md`](./single-column-revision.md): the calc-OFF single view is now a **single centered vertical column** (hero → stats → moves) resembling one damage-calc column. The description below is the original triptych design (kept for history).
