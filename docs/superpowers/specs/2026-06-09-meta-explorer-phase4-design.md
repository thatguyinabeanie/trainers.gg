...
Phase 4 adds three **metadata breakdown** views — usage sliced by a *player or
event* dimension rather than over time:
...
*understates* the data we have (top species per country) while *adding* deps and
...
*"Country data is available for RK9 and Limitless events only."* When the global
...
muted footer notes: *"Countries with fewer than N sampled players are hidden."*
...
**Empty / sparse states.**
- Source = trainers.gg → `EmptyState` (minimal variant): *"No country data for
  trainers.gg events. Country usage is available for RK9 and Limitless."*
- All countries below the floor (tiny slice) → *"Not enough data per country to
  show a regional breakdown yet."*
...
| Slope chart (rank lines across 3 columns) | rank *movement* between divisions | strong, but only shows rank not magnitude |
...
**Data scope + honesty.** `division` is **RK9 only**. Card caption: *"Division
data is available for RK9 events only."* Limitless and trainers.gg rows have NULL
...
falls below shows *"Not enough Junior data in this slice"* in place of its list,
...
**Empty / sparse states.**
- Source = limitless or trainers.gg → *"No division data for this source.
  Division (Masters/Seniors/Juniors) is available for RK9 events only."*
...
the *monotonic climb or fade* across the full placement spectrum. Two ways to
deliver that:
...
heatmap of 25 species × 4 bands shows the *whole field's* concentration pattern,
...
a band sum coherently — distinct from Phase 2's *cumulative* "top 10%" (which
includes the top 1%). This distinction is worth surfacing in a caption:
*"Bands are exclusive — 'Top 10%' here means players ranked 1–10% but outside the
top 1%."* The exact edges (1/10/25 vs e.g. 1/8/16) are the user's call.
...
columns get a tiny horizontal scroll *within the card only* (the status-pill-row
...
trainers.gg in-progress only) → *"No completed-event placement data in this
slice yet."* A band below the per-slice floor collapses into "Field" with a note.
...
- **Per-division denominator** uses the standard event denominator scoped to the
  ...
  *within each division*. Because `total_players` is recorded per
...
Generalizes the *concept* behind Phase 2's `get_usage_conversion` but does **not**
...
the **cumulative** top N%?" (a per-species conversion *rate*). RPC C answers
"within each **disjoint** placement band, what's the species' usage %?" (a
per-band *composition*). They share the `percent_rank()` machinery but produce
...
out of scope here. *Option noted for the user:* if Phase 2 is not yet locked, its
...
only if a view is *measured* slow** — matches the Phase 1 indexes-only,
