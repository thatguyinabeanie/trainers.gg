...
### Canonical source: `role-registry.ts`

The single source of truth for all role definitions is:

```text
apps/web/src/components/team-builder/v2/pickers/role-registry.ts
```

This file is shared by both the move picker and the species picker. It exports `MoveRoleGroup`, `MoveRolePreset`, `MOVE_ROLE_PRESETS`, `MOVE_ROLE_GROUP_LABELS`, `MOVE_ROLE_GROUP_ORDER`, `getRolesForMove`, `getRolesForSpecies`, `getRoleById`, and group color tokens. Do **not** create a separate `move-roles.ts` — import everything from `role-registry.ts`:

```ts
import {
  MOVE_ROLE_PRESETS, MOVE_ROLE_GROUP_LABELS, MOVE_ROLE_GROUP_ORDER,
  getRolesForMove, getRoleById,
  type MoveRoleGroup, type MoveRolePreset,
} from "./role-registry";
```

The reverse index is built lazily inside `role-registry.ts` (single pass over `MOVE_ROLE_PRESETS` on first call, memoized).
