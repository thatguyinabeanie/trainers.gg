...
- **Mobile data access (hybrid)** — Authenticated mobile reads stay direct Supabase (RLS via SecureStore session) — Supabase API requests are unlimited on Pro, and proxying through Vercel adds cost without benefit. Unauthenticated/public reads that broke under the Phase 2 anon-SELECT revoke move to cached `/api/v1` routes instead. `api-*` edge-function retirement and the full public-read cutover are deferred until mobile dev resumes. Shared query logic lives in `@trainers/supabase` — both web and mobile import from there. See `docs/decisions/2026-06-11-data-access-and-rls-decisions.md` for the hybrid rationale.
