...
7. **Keep `deno.json` import map in sync**: Every bare specifier reachable from any edge function must be mapped. See the "Import Map" section above.

## After Creating

1. **Verify import map**: Run the `deno cache` verification script above to confirm all functions bundle
