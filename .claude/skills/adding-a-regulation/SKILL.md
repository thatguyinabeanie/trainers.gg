...
- **Champions-exclusive Megas need a sprite fallback.** If any new Megas are Champions-exclusive (marked `// Champions-exclusive (custom — no @pkmn/dex entry)` in the reg file), add them to `CHAMPIONS_MEGA_SPRITE_URLS` in `packages/pokemon/src/sprites.ts` with the Serebii Legends Z-A URL — otherwise sprites render as gray boxes on the Showdown CDN (the CDN only hosts sprites for real-game species).
