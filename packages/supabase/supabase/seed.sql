...
BEGIN
  ...
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (cynthia_alt_id, 'The bond between trainer and Pokemon determines the outcome of every battle. Technical skill is important, but trust is everything.', NOW() - INTERVAL '1 day')
  RETURNING id INTO post_cynthia_2;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (cynthia_alt_id, 'Sinnoh Regional Qualifier was incredible! 128 players and the level of competition keeps rising. Proud of everyone who participated. 🏆', NOW() - INTERVAL '7 days')
  RETURNING id INTO post_cynthia_3;

  -- Ash's posts (enthusiastic, learning-focused)
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (ash_alt_id, 'Just hit Master Ball tier! Pikachu''s Light Ball set is still viable, you just gotta believe in your Pokemon!', NOW() - INTERVAL '30 minutes')
  RETURNING id INTO post_ash_1;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (ash_alt_id, 'VGC Weekly #41 was tough but we learned a lot. Koraidon matchup is still tricky - anyone have tips for the Charizard lead?', NOW() - INTERVAL '5 hours')
  RETURNING id INTO post_ash_2;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (ash_alt_id, 'New team idea: What if we ran Dragonite with Thunder Wave support instead of Dragon Dance? Slower setup but more consistent?', NOW() - INTERVAL '12 hours')
  RETURNING id INTO post_ash_3;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (ash_alt_id, 'Shoutout to the Pallet Town Trainers community for the practice matches today. This is why I love this game!', NOW() - INTERVAL '2 days')
  RETURNING id INTO post_ash_4;

  -- Karen's posts (philosophical, elite four vibes)
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (karen_alt_id, 'Strong Pokemon. Weak Pokemon. That is only the selfish perception of people. Truly skilled trainers should try to win with their favorites.', NOW() - INTERVAL '3 hours')
  RETURNING id INTO post_karen_1;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (karen_alt_id, 'Unpopular opinion: The "anti-meta" approach is often just as predictable as the meta itself. True innovation requires deeper understanding.', NOW() - INTERVAL '18 hours')
  RETURNING id INTO post_karen_2;

  -- Brock's posts (breeding/training focused)
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (brock_alt_id, 'Finally bred a perfect 6IV Geodude! Rock-types don''t get enough love in VGC but I''m determined to make them work.', NOW() - INTERVAL '4 hours')
  RETURNING id INTO post_brock_1;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (brock_alt_id, 'Pro tip: When breeding for competitive, always keep track of your egg groups. Makes building a diverse breeding pool much easier.', NOW() - INTERVAL '1 day' - INTERVAL '6 hours')
  RETURNING id INTO post_brock_2;

  -- Red's post (minimal, legendary status)
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (red_alt_id, '...', NOW() - INTERVAL '3 days')
  RETURNING id INTO post_red_1;

  -- Admin posts (platform/tournament focused)
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (admin_alt_id, '🎮 VGC Monthly Championship is LIVE! Check in now if you''re registered. Stream starting in 30 minutes.', NOW() - INTERVAL '2 hours' - INTERVAL '30 minutes')
  RETURNING id INTO post_admin_1;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (admin_alt_id, 'VGC Weekly #42 registration is now open! 32 player cap, Bo3 Swiss into Top 8. Sign up on trainers.gg before spots fill up!', NOW() - INTERVAL '1 day')
  RETURNING id INTO post_admin_2;
  
  INSERT INTO public.posts (alt_id, content, created_at)
  VALUES (admin_alt_id, 'Welcome to trainers.gg! We''re building the best platform for competitive Pokemon players. Let us know what features you want to see!', NOW() - INTERVAL '5 days')
  RETURNING id INTO post_admin_3;

  -- Reply posts (threading examples)
  INSERT INTO public.posts (alt_id, content, reply_to_id, created_at)
  VALUES (brock_alt_id, 'Congrats Ash! Light Ball Pikachu is underrated. The Fake Out + Volt Tackle pressure is real.', post_ash_1, NOW() - INTERVAL '25 minutes')
  RETURNING id INTO post_reply_1;
  
  INSERT INTO public.posts (alt_id, content, reply_to_id, created_at)
  VALUES (cynthia_alt_id, 'Well played! Consider running Follow Me support - it helps Pikachu set up safely.', post_ash_1, NOW() - INTERVAL '20 minutes')
  ...
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_cynthia_1, ash_alt_id),
    (post_cynthia_1, brock_alt_id),
    (post_cynthia_1, karen_alt_id),
    (post_cynthia_1, admin_alt_id);
  
  -- Cynthia's bond post
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_cynthia_2, ash_alt_id),
    (post_cynthia_2, brock_alt_id),
    (post_cynthia_2, red_alt_id);
  
  -- Ash's master ball post
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_ash_1, cynthia_alt_id),
    (post_ash_1, brock_alt_id),
    (post_ash_1, admin_alt_id);
  
  -- Ash's team idea post
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_ash_3, cynthia_alt_id),
    (post_ash_3, karen_alt_id);
  
  -- Karen's philosophy post (controversial = engagement)
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_karen_1, cynthia_alt_id),
    (post_karen_1, red_alt_id),
    (post_karen_1, admin_alt_id);
  
  -- Brock's breeding post
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_brock_1, ash_alt_id);
  
  -- Red's mysterious post (everyone likes it)
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_red_1, ash_alt_id),
    (post_red_1, cynthia_alt_id),
    (post_red_1, brock_alt_id),
    (post_red_1, karen_alt_id),
    (post_red_1, admin_alt_id);
  
  -- Admin's welcome post
  INSERT INTO public.post_likes (post_id, alt_id) VALUES
    (post_admin_3, ash_alt_id),
    (post_admin_3, cynthia_alt_id),
    (post_admin_3, brock_alt_id);
...
--
-- Social Feed:
-- - 17 posts total (including 2 replies)
