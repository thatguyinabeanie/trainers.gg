...
-- =============================================================================
-- This file creates test data for local development and preview branches.
-- It runs automatically after migrations via `supabase db reset` (local)
-- or via the Vercel prebuild script (preview branches).
--
-- IDEMPOTENT: This script can be run multiple times safely.
-- All INSERTs use ON CONFLICT DO NOTHING or existence checks.
--
...
DECLARE
  -- Early exit flag
  seed_already_exists boolean := false;
...
BEGIN
  ...
  -- ============================================================================
  -- Check for a known seed organization slug to determine if seeding already ran
  SELECT EXISTS(SELECT 1 FROM public.organizations WHERE slug = 'vgc-league') INTO seed_already_exists;
  
  IF seed_already_exists THEN
    RAISE NOTICE 'Seed data already exists (found vgc-league organization), skipping DO block.';
    RETURN;
  END IF;
  
  -- ============================================================================
  -- PREREQUISITE CHECK: Verify seed users were created
  -- ============================================================================
  -- Get alt IDs created by trigger (from auth.users insert)
  ...
  -- Verify at least the admin alt exists (if not, auth.users insert failed)
  IF admin_alt_id IS NULL THEN
    RAISE NOTICE 'Seed users not found (admin_alt_id is NULL). auth.users insert may have failed.';
    RAISE NOTICE 'Skipping remaining seed data that depends on seed users.';
    RETURN;
  END IF;
  ...
  -- ==========================================================================
  -- Organization Invitations (pending staff invitations)
  -- ==========================================================================
  -- These create pending invitations that users can accept/decline
  -- Used for testing the notification system and TO dashboard invite flow
  ...
  INSERT INTO public.organization_invitations (
    ...
    -- Sinnoh Champions invites Karen (pending) - Cynthia invites
    (sinnoh_champs_id, 'e5f6a7b8-c9d0-8e9f-2a3b-4c5d6e7f8a9b', 'c3d4e5f6-a7b8-6c7d-0e1f-2a3b4c5d6e7f', 'pending', NOW() + INTERVAL '7 days'),
    -- Sinnoh Champions invites Red (pending) - Cynthia invites
    (sinnoh_champs_id, 'f6a7b8c9-d0e1-9f0a-3b4c-5d6e7f8a9b0c', 'c3d4e5f6-a7b8-6c7d-0e1f-2a3b4c5d6e7f', 'pending', NOW() + INTERVAL '7 days'),
  ...
  RAISE NOTICE 'Seed data loading complete (including posts and social features)';
...
--
-- Organization Staff & Invitations:
-- - VGC League: Admin (owner), Ash (staff), Cynthia (staff)
-- - Pallet Town: Ash (owner), Admin (staff)
-- - Sinnoh Champions: Cynthia (owner), Admin (staff)
-- - Pending invitations for Brock (2), Karen (2), Red (1)
-- - Expired/declined examples for testing edge cases
--
