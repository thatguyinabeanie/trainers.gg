...
BEGIN
  ...
  IF v_main_id IS NULL OR v_vgc_id IS NULL OR v_draft_id IS NULL THEN
    RAISE NOTICE 'Admin alts not found, skipping admin match data';
    RETURN;
  END IF;

  -- Skip if admin already has matches
  IF EXISTS (SELECT 1 FROM tournament_matches WHERE alt1_id = v_vgc_id OR alt2_id = v_vgc_id) THEN
    RAISE NOTICE 'Admin match data already exists, skipping';
    RETURN;
  END IF;

  -- Insert matches directly as completed (round-locking trigger prevents
  -- the pending→completed UPDATE pattern on rounds that already have matches
  -- from 11_matches.sql). Then compute ELO via apply_elo_result() directly.
  ...
  -- Compute ELO ratings directly via apply_elo_result()
  -- Seed rating rows first
  INSERT INTO player_ratings (alt_id, format, rating, peak_rating, games_played, skill_bracket) VALUES
    (v_vgc_id,   'VGC',     1200, 1200, 0, 'beginner'),
    (v_vgc_id,   'overall', 1200, 1200, 0, 'beginner'),
    (v_main_id,  'VGC',     1200, 1200, 0, 'beginner'),
    (v_main_id,  'overall', 1200, 1200, 0, 'beginner'),
    (v_draft_id, 'VGC',     1200, 1200, 0, 'beginner'),
    (v_draft_id, 'overall', 1200, 1200, 0, 'beginner')
  ON CONFLICT DO NOTHING;

  -- Apply ELO for each match result
  -- admin_trainer_vgc: 7 wins, 3 losses across 2 tournaments
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1200); -- win
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 0.0, 1200); -- loss
  PERFORM apply_elo_result(v_vgc_id, 'overall', 0.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1210);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1210);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 0.0, 1220);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 0.0, 1220);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 1.0, 1190);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 1.0, 1190);
  PERFORM apply_elo_result(v_vgc_id, 'VGC', 0.0, 1230);
  PERFORM apply_elo_result(v_vgc_id, 'overall', 0.0, 1230);

  -- admin_trainer: 3 wins, 2 losses
  PERFORM apply_elo_result(v_main_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_main_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_main_id, 'VGC', 0.0, 1210);
  PERFORM apply_elo_result(v_main_id, 'overall', 0.0, 1210);
  PERFORM apply_elo_result(v_main_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_main_id, 'overall', 1.0, 1200);
  PERFORM apply_elo_result(v_main_id, 'VGC', 1.0, 1190);
  PERFORM apply_elo_result(v_main_id, 'overall', 1.0, 1190);
  PERFORM apply_elo_result(v_main_id, 'VGC', 0.0, 1220);
  PERFORM apply_elo_result(v_main_id, 'overall', 0.0, 1220);

  -- admin_trainer_draft: 2 wins, 2 losses
  PERFORM apply_elo_result(v_draft_id, 'VGC', 1.0, 1200);
  PERFORM apply_elo_result(v_draft_id, 'overall', 1.0, 1200);
