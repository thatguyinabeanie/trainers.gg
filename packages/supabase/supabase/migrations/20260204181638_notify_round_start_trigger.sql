...
-- =============================================================================
-- When a tournament round status changes to 'active', automatically send
-- notifications to all players in that round (both regular matches and byes).
--
-- Notifications include:
--   - Title: "{Tournament Name} — Round {N}"
--   - Body (regular): "Round {N}, Table {N} — vs {Opponent Display Name}"
--   - Body (bye): "You have a bye this round."
--   - Action URL: "/tournaments/{slug}/matches/{matchId}"
...
DROP TRIGGER IF EXISTS notify_round_start_trigger ON public.tournament_rounds;
CREATE TRIGGER notify_round_start_trigger
  AFTER UPDATE ON public.tournament_rounds
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_round_start();
