-- =============================================================================
-- Match check-in: modify start_round() to keep matches pending
-- =============================================================================
-- Previously, start_round() set all non-bye matches to 'active' immediately.
-- Now matches stay 'pending' so players must check in (confirm readiness)
-- before a match activates. The start_time is still set at round start —
-- this is the reference point for no-show timers.
--
-- Changes:
-- 1. Non-bye matches stay 'pending' (not 'active') with start_time = now()
-- 2. player1_match_confirmed and player2_match_confirmed reset to false
-- 3. Match games are still created so the game strip renders immediately
-- 4. BYE matches are still auto-completed as before
...
BEGIN
  ...
  -- 2. Set non-bye matches to pending with start_time (clock starts now)
