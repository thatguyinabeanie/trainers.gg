-- Update handle_new_user() to set is_public = true on the initial (main) alt.
--
-- Context: The main alt represents the user's primary identity and should
-- always be publicly visible. Previously, newly created main alts defaulted
-- to is_public = false, requiring a separate update step.
...
BEGIN
  ...
  -- Create alt record (main alt for the user) — always public
...
COMMENT ON FUNCTION public.handle_new_user() IS 'Trigger function for new user signup. Creates user and alt records with is_public=true on the main alt. Extracts username from OAuth metadata (username, preferred_username). Generates temp_<uuid> placeholder if no OAuth username available. Placeholder usernames must be replaced during onboarding.';
