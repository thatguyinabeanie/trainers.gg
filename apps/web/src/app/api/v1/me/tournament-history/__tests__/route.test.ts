...
const mockGetUserTournamentHistory = jest.fn();
const mockEnforceRateLimit = jest.fn();
const mockResolveApiAuth = jest.fn();

jest.mock("@trainers/supabase", () => ({
  getUserTournamentHistory: (...args: unknown[]) =>
    mockGetUserTournamentHistory(...args),
}));
...
const TOURNAMENT_HISTORY = [
  {
    ...
    organizationName: "Pallet League",
    organizationSlug: "pallet-league",
    startDate: "2026-04-01",
    endDate: "2026-04-01",
    format: "VGC 2026",
    altId: 5,
    altUsername: "ash_ketchum",
    altDisplayName: "ash_ketchum",
    placement: 2,
    wins: 5,
    losses: 1,
    ties: 0,
...
function makeRequest(options: { token?: string } = {}): NextRequest {
  ...
  return new NextRequest(
    "http://localhost:3000/api/v1/me/tournament-history",
    { headers }
  );
}
...
beforeEach(() => {
  jest.clearAllMocks();
  mockGetUserTournamentHistory.mockResolvedValue(TOURNAMENT_HISTORY);
...
describe("authentication", () => {
  it("returns 401 for an anonymous request (no cookie, no bearer)", async () => {
    ...
    expect(mockGetUserTournamentHistory).not.toHaveBeenCalled();
  });
});
...
describe("success", () => {
  it("returns 200 + the caller's history for a valid web cookie session", async () => {
    mockResolveApiAuth.mockResolvedValue(AUTHED_COOKIE);
    ...
    expect(mockGetUserTournamentHistory).toHaveBeenCalledWith(COOKIE_SUPABASE);
  });

  it("returns 200 + the caller's history for a valid mobile Bearer token", async () => {
    ...
    expect(mockGetUserTournamentHistory).toHaveBeenCalledWith(BEARER_SUPABASE);
  });

  it("returns 200 with an empty array when user has no history", async () => {
    mockResolveApiAuth.mockResolvedValue(AUTHED_COOKIE);
    mockGetUserTournamentHistory.mockResolvedValue([]);
