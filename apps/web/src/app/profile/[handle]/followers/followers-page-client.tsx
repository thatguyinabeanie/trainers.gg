...
import { useProfile, useFollowers, flattenFollowPages } from "@/hooks/bluesky";
...
/**
 * Client component for the followers page.
 * Displays a list of users who follow the specified profile.
 */
export function FollowersPageClient({ handle }: FollowersPageClientProps) {
  ...
  const followers = flattenFollowPages(followersData?.pages);
      ...
      <header className="bg-background/95 supports-[backdrop-filter]:bg-background/60 border-border sticky top-0 z-10 border-b backdrop-blur">
        <div className="flex h-14 items-center gap-4 px-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-foreground truncate font-bold">Followers</h1>
            {!isProfileLoading && (
        ...
        {isFollowersLoading ? (
          ...
          <>
            {followers.map((follower) => (
              <ProfileListItem key={follower.did} profile={follower} />
            ))}
