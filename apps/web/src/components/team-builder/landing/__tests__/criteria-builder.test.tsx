 ...
 * available in JSDOM. We replace the whole compound component with simple
 * HTML elements so tests can drive value changes via a native <select>.
 *
 * The mock renders:
 *   <div data-testid="select-root" data-value={value}>
 *     <select aria-label={...} value={value} onChange={...}>
 *       {children rendered into SelectContent/SelectItem}
 *     </select>
 *   </div>
 *
...
// We collect children rendered inside SelectContent so the root Select can
// build <option> elements from them. The simplest JSDOM-friendly strategy is
// to render the content directly inside a real <select>.
...
jest.mock("@/components/ui/select", () => {
  ...
  function Select({
    ...
    // We use a ref + state so that SelectTrigger can register its aria-label
    // before SelectContent renders.  The simplest approach: collect ariaLabel
    // from the trigger via context mutation during render.
    const [ariaLabel, setAriaLabel] = React.useState<string | undefined>(
      undefined
    );
    return (
      <SelectContext.Provider
        value={{ value, onValueChange, ariaLabel: ariaLabel as string | undefined }}
      >
  ...
  function SelectContent({ children }: { children: React.ReactNode }) {
    ...
    // We piggyback on React.Children for simple flat child lists.
