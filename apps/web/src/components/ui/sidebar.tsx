...
function SidebarProvider({
  ...
  React.useEffect(() => {
    const match = document.cookie.match(
      new RegExp(`(?:^|; )${SIDEBAR_WIDTH_COOKIE}=(\\d+)`)
    );
    if (match) {
  ...
  // Persist sidebar state to a cookie whenever it changes.
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    startWidth.current = sidebarEl?.offsetWidth ?? 256;
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background relative flex min-w-0 flex-1 flex-col md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
...
function SidebarMenuButton({
  ...
  // Build the button/link element using the caller's render prop (e.g. <Link>)
