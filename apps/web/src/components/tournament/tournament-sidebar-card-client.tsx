...
/** Registration status payload shape returned by `getRegistrationStatus`. */
export type SidebarRegistrationStatus = NonNullable<
  Awaited<ReturnType<typeof getRegistrationStatus>>
>;

/** Check-in status payload shape returned by `getCheckInStatus`. */
export type SidebarCheckInStatus = NonNullable<
  Awaited<ReturnType<typeof getCheckInStatus>>
>;

/** Check-in stats payload shape returned by `getCheckInStats`. */
export type SidebarCheckInStats = NonNullable<
  Awaited<ReturnType<typeof getCheckInStats>>
>;

export interface TournamentSidebarCardProps {
  tournamentId: number;
...
interface TournamentSidebarCardClientProps extends TournamentSidebarCardProps {
  /** Registration status fetched server-side (service-role, SSR). */
  initialRegistration: SidebarRegistrationStatus;
  /** Caller's check-in status fetched server-side (null for logged-out). */
  initialCheckIn: SidebarCheckInStatus | null;
  /** Public check-in stats fetched server-side. */
  initialCheckInStats: SidebarCheckInStats | null;
}
...
export function TournamentSidebarCardClient({
  tournamentId,
  tournamentSlug,
  tournamentName,
  gameFormat,
  ...
  // ---- Server-fetched data (props) ----
  //
  // Registration counts + the caller's status are fetched server-side via
  // service-role (Phase 2 Task 9 — base-table SELECT revoked for anon/authed).
  // There is no browser read or realtime subscription anymore; after the viewer
  // mutates their own registration we call `router.refresh()` to re-run the
  // server fetch and receive fresh props. Per D1, a live check-in board (other
  // users' changes in realtime) is not a needed feature.
  const registrationStatus = initialRegistration;
  const checkInStatus = initialCheckIn;
  const checkInStats = initialCheckInStats;

  /** Re-run the server component fetch to pick up the viewer's own changes. */
  const refreshRegistration = () => router.refresh();
