...
#   • E2E Tests, Lighthouse, Bundle Analysis, Wait-for-Preview.
#     These are reported as SKIPPED below with the reason.
#
# USAGE
#   pnpm ci:local                      # everything (fast; no coverage gate)
...
#   ./scripts/ci-local.sh                  # direct invocation
#
# Extra args are forwarded to the `turbo run` invocation (e.g. --filter=...).
